...
union 함수는 두 노드를 연결해 줄 뿐, 두 노드와 기존에 연결되어 있던 노드들까지 업데이트 해주지는 않는다.   
그러므로 마지막에 확인할 때 반드시 parent가 아닌 root(find 함수의 return 값)으로 비교를 해야한다!   
여기서 디버깅 시간이 오래 걸렸다..ㅠㅠ

**❗예시)**   
3번과 7번을 연결하고, 3번과 5번을 연결한다.   
그 다음에 1번과 5번을 연결하면?   
3번과 7번은 여전히 parent가 3으로, root를 찾으면 1인데 parent가 아직 업데이트가 안됨을 알 수 있다.
   


### 성능 요약

메모리: 231260 KB, 시간: 844 ms
